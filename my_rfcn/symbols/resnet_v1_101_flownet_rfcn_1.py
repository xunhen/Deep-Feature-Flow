...
class resnet_v1_101_flownet_rfcn(Symbol):
    ...
    def get_flownet(self, img_cur, img_ref):
        ...
        # changed by wjc
        Convolution5_residual_bias = mx.sym.Variable(name='Convolution5_residual_bias', lr_mult=0.0)
        Convolution5_residual = mx.symbol.Convolution(name='Convolution5_residual', data=Concat5, num_filter=1024,
                                                      pad=(0, 0),
                                                      kernel=(1, 1), stride=(1, 1),
                                                      bias=Convolution5_residual_bias, no_bias=False)
        return Convolution5 * 2.5, Convolution5_scale, Convolution5_residual
    ...
    def get_train_symbol(self, cfg):
        ...
        wrap_feat_weight = mx.sym.Variable(name='wrap_feat_weight')
        ...
        # changed by wjc
        conv_feats = self.get_resnet_v1(mx.sym.concat(data_ref, data, dim=0))
        conv_feat_ref, conv_feat_data = mx.sym.SliceChannel(conv_feats, axis=0, num_outputs=2)

        flow, scale_map, residual_map = self.get_flownet(data, data_ref)
        flow_grid = mx.sym.GridGenerator(data=flow, transform_type='warp', name='flow_grid')
        warp_conv_feat = mx.sym.BilinearSampler(data=conv_feat_ref, grid=flow_grid, name='warping_feat')

        # wjc the scale_map is redisual
        scale_map = mx.sym.BilinearSampler(data=scale_map, grid=flow_grid, name='warping_scale')
        residual_map = mx.sym.BilinearSampler(data=residual_map, grid=flow_grid, name='warping_residual')
        warp_conv_feat = warp_conv_feat * scale_map + residual_map
        feat_loss_ = mx.sym.sum(mx.sym.smooth_l1(name='feat_loss_', scalar=3.0,
                                                 data=warp_conv_feat - mx.sym.stop_gradient(
                                                     conv_feat_data)), axis=1)
        feat_loss_ = feat_loss_ * wrap_feat_weight

        feat_loss = mx.sym.MakeLoss(name='feat_loss', data=feat_loss_, normalization='valid')

        select_conv_feat = mx.sym.take(mx.sym.Concat(*[warp_conv_feat, conv_feat_ref], dim=0), eq_flag)
        ...
        group = mx.sym.Group(
            [rpn_cls_prob, rpn_bbox_loss, cls_prob, bbox_loss, mx.sym.BlockGrad(rcnn_label), feat_loss])
        self.sym = group
        return group
    ...
    def init_weight(self, cfg, arg_params, aux_params):
        ...
        arg_params['Convolution5_residual_weight'] = mx.nd.zeros(
            shape=self.arg_shape_dict['Convolution5_residual_weight'])
        arg_params['Convolution5_residual_bias'] = mx.nd.zeros(shape=self.arg_shape_dict['Convolution5_residual_bias'])
